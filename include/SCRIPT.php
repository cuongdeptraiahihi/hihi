...
<script src="https://localhost/www/TDUONG/js/materialize.js"></script>
<script src="https://localhost/www/TDUONG/js/county.js"></script>
<script src="https://localhost/www/TDUONG/js/jquery.fireworks.js"></script>
<script src="https://localhost/www/TDUONG/js/jquery.typewatch.js"></script>
<script type="text/javascript">
	$(document).ready(function() {
        ...
        setTimeout(function () {
            $.ajax({
                ...
                success: function (result) {
                    $(result).insertBefore($("#NAVBAR ul li#li-temp"));
                    $("li#tb-icon").append($("li#li-temp").html());
                    $("li#li-temp").remove();
                }
            });
        },1000);
		...
		<?php
            $nhayde=false;
            if(!isset($_SESSION["nhay_de"]) && isset($_SESSION["ID_HS"]) && isset($_SESSION["lmID"])) {
                $_SESSION["nhay_de"] = check_nhayde_G($_SESSION["ID_HS"],$_SESSION["lmID"]);
            }
            if(isset($_SESSION["nhay_de"])) {
                $nhayde = $_SESSION["nhay_de"];
            }
			if($nhayde) {
				echo"$('body').fireworks();";	
			}
			if(!isset($_SESSION["count_time"])) {
                $_SESSION["count_time"] = get_thi_daihoc($_SESSION["lop"]);
            }
            $count_time = $_SESSION["count_time"];
        ?>
