                ...
                $("input.xong").click(function() {
                    ...
                    if($.isNumeric(sttID) && sttID!=0 && confirm("Bạn có chắc chắn không?")) {
                        ...
                        $.ajax({
                            ...
                            data: "sttID=" + sttID,
                            type: "post",
                            url: "http://localhost/www/TDUONG/trogiang/xuly-thachdau/",
                ...
                $("input.xoa").click(function() {
                    ...
                    if($.isNumeric(sttID) && sttID!=0 && confirm("Bạn có chắc chắn không?")) {
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                del_tr.fadeOut("fast").removeClass("stt");
                                $("#BODY").css("opacity","1");
                                $("#popup-loading").fadeOut("fast");
                                stt();
                            }
    ...
    <body>
        <div class="popup" id="popup-add" style="width:40 %;left:30%;">
            ...
            <div style="width:90%;margin:15px auto 15px auto;">
                <input style="padding:15px;margin-bottom:3px;" id="maso-add" class="input" autocomplete="off" placeholder="Mã học sinh" value="<?php echo $data0["cmt"]; ?>" />
                <input style="padding:15px;" id="content-add" class="input" autocomplete="off" placeholder="Nội dung" style="margin-top:10px;" />
            </div>
            ...
            if(isset($_POST["search-ok"])) {
                ...
                if(valid_maso($maso)) {
                    ...
                    if(is_numeric($id) && $id!=0) {
                        header("location:http://localhost/www/TDUONG/trogiang/level-thuong/$id/");
                        exit();
                    } else {
                        header("location:http://localhost/www/TDUONG/trogiang/level-thuong/");
                        exit();
                    }
                } else {
                    header("location:http://localhost/www/TDUONG/trogiang/level-thuong/");
...
<!--                                <li><span><span style="font-size:8px">&#9899;</span> +0k nếu chuyển tạm sang ca VẮNG hơn</span></li>-->
                	...
                	<div id="main-person">
                		<h1 style="line-height:98px;">QUẢN LÝ THẺ MIỄN PHẠT</h1>
                        <div class="clear"></div>
                   	</div>
                    <div id="main-avata">
                ...
                <div class="main-div animated bounceInUp" id="main-mid">
                    <div id="main-info">
                        <div class="main-1-left back" style="margin-right:0;max-height:none;width: 100%;float: none;">
                            <div>
                                <p class="main-title">THẺ MIỄN PHẠT</p>
                            </div>
                            <table class="table table-tkb" id="list-lich">
                                <tr>
                                    <form action="http://localhost/www/TDUONG/trogiang/level-thuong/" method="post">
                                        <td colspan="3" style="line-height:72px;"><span>Tìm kiếm</span></td>
                                        <td><input type="text" class="input" style="margin-top:15px;" placeholder="Mã học sinh" id="search-hs" name="search-hs" value="<?php echo $data0["cmt"]; ?>" /></td>
                                        <td><input style="margin-top:15px;" type="submit" class="submit" value="Tìm" name="search-ok" /></td>

                                    </form>
                                    <td><input style="margin-top:15px;" type="submit" class="submit" value="Thêm" id="add-thuong" /></td>
                                </tr>
                                <tr>
                                ...
                                $dem=0;
                                if($hsID!=0) {
                                    $query = "SELECT h.ID_HS,h.cmt,h.fullname,l.*,o.name FROM log AS l INNER JOIN hocsinh AS h ON h.ID_HS=l.ID_HS INNER JOIN hocsinh_mon AS m ON m.ID_HS=l.ID_HS INNER JOIN lop_mon AS o ON m.ID_LM=o.ID_LM WHERE l.ID_HS='$hsID' AND l.content NOT LIKE '%(Đã nhận)' AND (l.type='the-mien-phat') ORDER BY l.datetime DESC,h.cmt ASC";
                                } else {
                                    $query = "SELECT h.ID_HS,h.cmt,h.fullname,l.*,o.name FROM log AS l INNER JOIN hocsinh AS h ON h.ID_HS=l.ID_HS INNER JOIN hocsinh_mon AS m ON m.ID_HS=l.ID_HS INNER JOIN lop_mon AS o ON m.ID_LM=o.ID_LM WHERE l.content NOT LIKE '%(Đã nhận)' AND (l.type='the-mien-phat') ORDER BY l.datetime DESC,h.cmt ASC";
                                }
                                $result=mysqli_query($db,$query);
                                while($data=mysqli_fetch_assoc($result)) {
                                    if($dem%2!=0) {
                                        echo"<tr class='stt' style='background:rgba(209,219,189,0.35);'>";
                                    } else {
                                        echo"<tr class='stt'>";
                                    }
                                    ?>
                                    <td class="stt"><span><?php echo ($dem+1);?></span></td>
                                ...
                                if($hsID!=0) {
                                    $query = "SELECT h.ID_HS,h.cmt,h.fullname,l.*,o.name FROM log AS l INNER JOIN hocsinh AS h ON h.ID_HS=l.ID_HS INNER JOIN hocsinh_mon AS m ON m.ID_HS=l.ID_HS INNER JOIN lop_mon AS o ON m.ID_LM=o.ID_LM WHERE l.ID_HS='$hsID' AND l.content LIKE '%(Đã nhận)' ORDER BY l.datetime DESC,h.cmt ASC";
                                } else {
                                    $query = "SELECT h.ID_HS,h.cmt,h.fullname,l.* FROM log AS l INNER JOIN hocsinh AS h ON h.ID_HS=l.ID_HS INNER JOIN hocsinh_mon AS m ON m.ID_HS=l.ID_HS INNER JOIN lop_mon AS o ON m.ID_LM=o.ID_LM WHERE l.content LIKE '%(Đã nhận)' ORDER BY l.datetime DESC,h.cmt ASC";
                                }
                                $result=mysqli_query($db,$query);
                                while($data=mysqli_fetch_assoc($result)) {
                                    if($dem%2!=0) {
                                    ...
                                    ?>
                                    <td class="stt"><span><?php echo ($dem+1);?></span></td>
                                    <td><span><?php echo $data["fullname"]."(".$data["cmt"].")"; ?></span></td>
                                    <td><span><?php echo $data["name"]; ?></span></td>
                                    <td><span><?php echo $data["content"]; ?></span></td>
                                    <td><span><?php echo format_datetime($data["datetime"]); ?></span></td>
                                    <td></td>
