...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>       
        ...
        <title>QUẢN LÝ TIỀN CHI, THƯỞNG</title>
		...
		$(document).ready(function() {
            ...
            $("#MAIN #main-mid .status .table tr td input.add").click(function() {
                ...
                if(note != "" && price != "") {
                    $.ajax({
                        async: true,
                        data: "hsID2=" + <?php echo $hsID; ?> + "&note2=" + note + "&price2=" + price + "&action=thuong",
                        type: "post",
                        url: "http://localhost/www/TDUONG/admin/xuly-taikhoan/",
                        success: function (result) {
                            del_tr.css("opacity","1");
                	...
                	<div>
                    	<div class="status">
                            	<table class="table">
                                    <?php
                                        if($hsID != 0) {
                                            echo"<tr>
                                                ...
                                                <td><input type='text' class='input note_td' placeholder='Nội dung thưởng' /></td>
                                    ...
                                    <tr><td colspan="5"><span>Đã rút</span></td></tr>
                                    <?php
                                        $total1=0;
                                        $result=get_rut_hocsinh($hsID);
										$dem=0;
										while($data=mysqli_fetch_assoc($result)) {
											...
											echo"<td><span>".format_date($data["date"])."</span></td>
												...
												<td><input type='submit' class='submit edit2' data-idVAO='$data[ID_VAO]' value='Sửa' /><input type='submit' class='submit delete2' data-idVAO='$data[ID_VAO]' value='Xóa' /></td>
											</tr>";
                                    ...
                                    <tr><td colspan="5"><span>Cần chi</span></td></tr>
                                    <?php
                                    $total1=0;
                                    $result=get_thuong_hs_con2($hsID);
                                    $dem=0;
                                    while($data=mysqli_fetch_assoc($result)) {
                                        if($dem%2!=0) {
                                            echo"<tr style='background:#D1DBBD;'>";
                                        ...
                                        echo"<td><span>".format_date($data["date"])."</span></td>
												...
												<td><input type='text' class='input note_td' value='$data[note]' /></td>
												<td><input type='text' class='input price_td' value='".format_price($data["price"])."' style='text-align:center;' /></td>
												<td><input type='submit' class='submit edit' data-idRA='$data[ID_RA]' value='Sửa' /><input type='submit' class='submit delete' data-idRA='$data[ID_RA]' value='Xóa' /></td>
											</tr>";
                                        $total1=$total1+$data["price"];
                                        $dem++;
                                    }
                                    if($dem==0) {
                                        echo"<tr><td colspan='4'><span>Không có dữ liệu</span></td></tr>";
                                    }
                                    ?>
                                    <tr style="background:#3E606F;">
                                        <th colspan="3"><span>Tổng cộng</span></th>
