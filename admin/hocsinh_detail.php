    ...
    <head>       
		...
		$(document).ready(function() {
			...
			$("#hs-date").change(function() {
				...
				$("#xem-hs-date").attr("onclick","location.href='http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/"+ngay+"/'");
			});
...
//						alert(result);
						if(result!="none") {
							...
							$("#add-thuong").attr("href","http://localhost/www/TDUONG/admin/thuong/"+obj.hsID+"/");
							$("#add-phat").attr("href","http://localhost/www/TDUONG/admin/phat/"+obj.hsID+"/");
				...
				if((hsID!=0 || (hsID==0 && lop!=0)) && name!="" && birth!="" && (gender==0 || gender==1) && ajax_data!="") {
					...
					$.ajax({
						...
						success: function(result) {
                            ...
                            $(".add-new").val(obj.msg);
                            alert(obj.msg);
                            window.location.href="http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/ma/" + obj.cmt + "/";
							$("#BODY").css("opacity","1");
							$("#popup-loading").fadeOut("fast");
						}
					});
            ...
            <?php
                if(isset($_POST["xem-hs-date"])) {
                    if(isset($_POST["hs-date"])) {
                        $date=$_POST["hs-date"];
                        header("location:http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/$date/");
                        exit();
                    } else {
                        header("location:http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/");
                        exit();
                    }
                }
            ?>
...
<!--                                <tr style="background:#3E606F;"><th colspan="3"><span>Ghi chú chính</span><a href="javascript:void(0)" id="special-note" style="float:right;margin-right: 10px;color:yellow;text-decoration: underline;">Đặc biệt</a></th></tr>-->
                            ...
                            <table class="table" style="margin-top:25px;">
                                <form action="http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/" method="post">
                                    <tr>
                                        ...
                                        function display_hocsinh($query, $is, $dem) {
                                            ...
                                            while($data=mysqli_fetch_assoc($result)) {
                                                ...
                                                if($dem%2!=0) {
                                                    ...
                                                    echo"<td><a href='http://localhost/www/TDUONG/admin/hoc-sinh-chi-tiet/ma/$data[cmt]/'>$data[cmt]</a></td>
