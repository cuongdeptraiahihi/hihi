<div id="NAVBAR">
	<ul>
        <?php
			...
			while($datax=mysqli_fetch_assoc($resultx)) {
				if($monID==$datax["ID_MON"]) {
					echo"<li style='background:#FFF;'><a href='http://localhost/www/TDUONG/admin/hoc-sinh-tong-quan/$hsID/$monID/' title='Môn $datax[name]'>$datax[name]</a></li>";
				} 
			}
		?>
        ...
        <li style='background:#FFF;'><a href="http://localhost/www/TDUONG/admin/background/" title="Quay ra"><i class="fa fa-sign-out"></i></a></li>
    </ul>
</div>

<div class="popup" id="popup-anh" style="max-height:500px;overflow-y:auto;top:10%">
	<form action="http://localhost/www/TDUONG/admin/change-back/<?php echo "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"]; ?>/" method="post" enctype="multipart/form-data">
...
<div class="popup" id="popup-opa" style="top:10%;">
	<form action="http://localhost/www/TDUONG/admin/change-opa/<?php echo "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"]; ?>/" method="post">
