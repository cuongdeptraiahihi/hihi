	...
	if(isset($_POST["hsID"])) {
        ...
        if($_SESSION["mobile"]==0) {
            echo "<tr class='tien-hoc'>
                <td style='text-align:center'><a href='http://localhost/www/TDUONG/admin/nghi-dai/$hsID/' target='_blank'>Nghỉ dài hạn</a></td>
                <td style='text-align:center'><a href='http://localhost/www/TDUONG/admin/sua-de/$hsID/' target='_blank'>Đề kiểm tra</a></td>
...
//        </tr>";
        echo"<tr style='background: #3E606F;'>   
            <th colspan='3'><a style='text-transform: uppercase;color:#FFF;text-decoration: underline;' href='http://localhost/www/TDUONG/admin/level-thuong/$hsID/' target='_blank'>Thẻ miễn phạt</a></th>
        </tr>";
        $query="SELECT l.ID_LM,l.name,l.ID_MON,m.de,m.date_in,n.date FROM lop_mon AS l LEFT JOIN hocsinh_mon AS m ON m.ID_HS='$hsID' AND m.ID_LM=l.ID_LM LEFT JOIN hocsinh_nghi AS n ON n.ID_HS='$hsID' AND n.ID_LM=l.ID_LM ORDER BY l.ID_MON ASC,l.ID_LM ASC";
		...
		while($data=mysqli_fetch_assoc($result)) {
			...
			$mon_name=get_mon_name($data["ID_MON"]);
			...
			echo"<tr class='mon-big' data-monID='$data[ID_LM]'>
				<td style='width:25%;'><input type='text' class='input date-in' value='$date_in' /><input type='text' style='background:red;color:#FFF;margin-top:5px;' placeholder='Ngày nghỉ hẳn' class='input date-out' value='$date_out' /></td>
				<td style='width:25%;'><a href='https://localhost/www/TDUONG/dang-nhap/$hsID/".get_sdt_phuhuynh($hsID)."/$data[ID_LM]/' target='_blank'>$mon<i class='fa fa-eye' style='margin-left:5px;'></i></a></td>
				<td style='width:50%;'>
				    ...
				    <div style='float:right;'>";
                        ...
                        echo"<a href='http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-hs/$data[ID_LM]/2/$cmt/' style='margin-top:5px;display:block;' target='_blank' class='lich-su-doi-ca'>Lịch sử điểm</a>
                        ...
                        <a href='http://localhost/www/TDUONG/admin/hoc-sinh-diem-danh/$data[ID_LM]/$data[ID_MON]/3/$hsID/' style='margin-top:5px;display:block;' target='_blank' class='lich-su-doi-ca'>Điểm danh</a>
                        <a href='http://localhost/www/TDUONG/admin/hoc-sinh-lich-hoc/$hsID/$data[ID_LM]/' style='margin-top:5px;display:block;' target='_blank'>Đổi ca</a>
                        <a href='http://localhost/www/TDUONG/admin/hoc-sinh-video/$hsID/$data[ID_LM]/' style='margin-top:5px;display:block;' target='_blank'>Hỗ trợ video</a></td>
                    </div>
                </td>
			</tr>";
            $n=count($data_array);
			...
			while($data3=mysqli_fetch_assoc($result3)) {
				echo"<tr class='mon-lich mon-lich_$data[ID_LM]' $style>
					...
					<td colspan='2'>";
					if($style=="") {
						echo"<select class='input chose-ca' style='height:auto;width:100%'>
							<option value='0'>Chọn ca</option>";
							$result2=get_cahoc_cum_lop($data3["ID_CUM"], $data["ID_LM"], $data["ID_MON"]);
							while($data2=mysqli_fetch_assoc($result2)) {
								echo"<option value='$data2[ID_CA]'";if($n>0 && isset($data_array[$data2["cum"]]) && $data2["ID_CA"]==$data_array[$data2["cum"]]) {echo" selected='selected'";}echo" data-cum='$data2[cum]'>".$thu_string[$data2["thu"]-1].", ca $data2[gio]</option>";
							}
						echo"</select>";
					} else {
						if($n>0 && isset($data_array[$data3["ID_CUM"]])) {
							$result4=get_info_ca($data_array[$data3["ID_CUM"]]);
							$data4=mysqli_fetch_assoc($result4);
							echo"<span>".$thu_string[$data4["thu"]-1].", ca $data4[gio]</span>";
						} else {
			...
			echo"<tr class='mon-lich mon-lich_$data[ID_LM]' $style>
				...
				if($style=="") {
					echo"<select class='input chose-ca' style='height:auto;width:100%'>
						...
						while($data2=mysqli_fetch_assoc($result2)) {
							echo"<option value='$data2[ID_CA]'";if($n>0 && isset($data_array[$data2["cum"]]) && $data2["ID_CA"]==$data_array[$data2["cum"]]) {echo" selected='selected'";}echo" data-cum='$data2[cum]'>$stt - ".$thu_string[$data2["thu"]-1].", ca $data2[gio]</option>";
						    $stt++;
						}
					echo"</select>";
				} else {
...
//                                $string .= " <i class='btn-edit-price fa fa-pencil' style='cursor: pointer;'></i><input style='width: 60px;margin-left: 5px;padding: 5px;display: none;' class='input edit-price' data-thang='$nam-$thang' data-lmID='$data[ID_LM]' value='$data1[content]' /> <i class='del-edit-price fa fa-times' data-oID='$data1[ID_O]' style='cursor: pointer;'></i></span><br />";
    ...
    if(isset($_POST["hsID_note"])) {
        ...
        $data=mysqli_fetch_assoc($result);
        echo"<tr class='tr-note' style='background:#3E606F;'>
        ...
        <tr class='tr-note' style='background:#3E606F;'>
            <th colspan='3'><span>Ghi chú chính</span></th>
        </tr>
        <tr class='tr-note'>
            <td colspan='3'><span data-date='0'></span><textarea style='resize:none;box-sizing: border-box;overflow: hidden;' placeholder='Tự động lưu' data-hsID='$hsID' class='input note-day'>".str_replace("<br />","\n",get_note_hs($hsID))."</textarea></td>";
...
//                    echo"</td>
            ...
            echo"<tr class='tr-note'>
                ...
                <td><textarea style='resize:none;box-sizing: border-box;overflow: hidden;' placeholder='Tự động lưu' rows='1' data-hsID='$hsID' class='input note-day'>".str_replace("<br />","\n",$data["note"])."</textarea></td>";
//                <td>";
//                if($data["hot"]==1) {
//                    echo "<input type='submit' data-nID='$data[ID_STT]' class='submit check-chuy is_chuy' style='background:red;' value='New' />";
//                } else {
