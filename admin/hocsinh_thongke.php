		...
		{ 
              ...
              if($_GET["loai"]!=2) { ?>
		  {
			  ...
			  <?php
			  	$i=count($to_none)-1;
				while($i>=0) {
					echo"{ label: '".$to_none[$i]["ngay"]."', y: ".$to_none[$i]["num"].", indexLabel: '{y}', content: 'http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-buoi/$lmID/".$to_home[$i]["buoi"]."/nghi/'},";
					$i--;
				}
              ?>
			  ]
              ...
              if($_GET["loai"]!=2) { ?>
		  {
			  ...
			  <?php
			  	$i=count($to_home)-1;
				while($i>=0) {
					echo"{ label: '".$to_home[$i]["ngay"]."', y: ".$to_home[$i]["num"].", indexLabel: '{y}', content: 'http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-buoi/$lmID/".$to_home[$i]["buoi"]."/tohome/'},";
					$i--;
				}
              ?>
			  ]
            ...
            if($_GET["loai"]!=2) { ?>
              {
                  ...
                  dataPoints: [
                      ...
                      $i = count($to_huy)-1;
                      while ($i >= 0) {
                          echo "{ label: '" . $to_huy[$i]["ngay"] . "', y: " . $to_huy[$i]["num"] . ", indexLabel: '{y}', content: 'http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-buoi/$lmID/" . $to_huy[$i]["buoi"] . "/tohuy/'},";
                          $i--;
                      }
                      ?>
                  ]
              ...
              if($_GET["loai"]!=2) { ?>
		  {
			  ...
			  dataPoints: [
				  ...
				  $i=count($diemtbY)-1;
				  while($i>=0) {
					  $content="http://localhost/www/TDUONG/admin/pho-diem/".$diemtbY[$i]["ngay"]."/Y/$lmID/";
		  ...
		  {        
			...
			<?php
				$i=count($diemtbB)-1;
				while($i>=0) {
				    $content="http://localhost/www/TDUONG/admin/pho-diem/".$diemtbB[$i]["ngay"]."/B/$lmID/";
		  ...
		  {        
			...
			<?php
				$i=count($diemtbG)-1;
				while($i>=0) {
                    $content="http://localhost/www/TDUONG/admin/pho-diem/".$diemtbG[$i]["ngay"]."/G/$lmID/";
					...
					if(dem>=1) {
						...
						if ($.isNumeric(oID) && $.isNumeric(buoiID) && buoiID != 0 && $.isNumeric(hsID)) {
							...
							$.ajax({
								...
								type: "post",
								url: "http://localhost/www/TDUONG/admin/xuly-nhapdiem/",
...
//				echo $_SESSION["bieudo"];
					...
					if(isset($_POST["set-ok"])) {
						...
						if($loai==0) {
							header("location:http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/$lmID/$loai/");
							exit();
						} else if($loai==1) {
							if(isset($_POST["ma1"])) {
								$ma1=$_POST["ma1"];
							...
							if($ma1 && $ma2) {
								header("location:http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/$lmID/$loai/$ma1/$ma2/");
								exit();
							} else {
								header("location:http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/$lmID/0/");
								exit();
							}
						} else if($loai==2) {
							if(isset($_POST["ma"])) {
							...
							if($ma) {
								header("location:http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/$lmID/$loai/$ma/");
								exit();
							}
						} else {
							header("location:http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/$lmID/$loai/");
                	...
                	<div>
                        ...
                        <?php } } ?>
                            <div class="main-bot" style="display:block;">
								<?php if($_SESSION["mobile"]==0) { ?>
                            	<!--<form action="http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke/<?php echo $lmID; ?>/" method="post">
                        ...
                        <div class="page-number">
                            ...
                            <?php
								if($current!=1) {
									...
									echo"<li><a href='http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-page/$lmID/$loai/page-$prev/'><</a></li>";
								}
								for($page=1;$page<=$sum_page;$page++) {
									$begin=($page-1)*$display;
									if($page==$current) {
										echo"<li><a href='http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-page/$lmID/$loai/page-$begin/' style='font-weight:bold;text-decoration:underline;'>$page</a></li>";
									} else {
										echo"<li><a href='http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-page/$lmID/$loai/page-$begin/'>$page</a></li>";
									}
								}
								if($current!=$sum_page) {
									$next=$position+$display;
									echo"<li><a href='http://localhost/www/TDUONG/admin/hoc-sinh-thong-ke-page/$lmID/$loai/page-$next/'>></a></li>";
								}
							?>
                            </ul>
                        </div>
