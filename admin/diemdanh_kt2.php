    ...
    <head>       
		...
		$(document).ready(function() {
			function a() {
				...
				$("#hs-info, #ca-regis, #ca-result").html("");
                $("#ca-result").css("background", "none");
                $("#co-dung").prop("checked", !0);
			}
			...
			function get_form_de_thi(hsID, buoiID, made) {
                $("#popup-loading").fadeIn("fast");
                $("#BODY").css("opacity", "0.3");
                clean_chuyende();
                $.ajax({
                    async: true,
                    data: "hsID8=" + hsID + "&buoiID8=" + buoiID + "&made8=" + made,
                    type: "post",
			...
			function get_info_buoikt(hsID, buoiID) {
				...
				$.ajax({
					...
					success: function(result) {
                        ...
                        get_form_de_thi(hsID,buoiID,$("#hs-made").val());
						$("#BODY").css("opacity", "1");
						$("#popup-loading").fadeOut("fast");
					}
				});
			...
			/*}),*/ $("#search-hs").click(function() {
                    a();
                    if($("#select-mon").val()==0 || $("#select-ca").val()==0) {
                        alert("Bạn hãy chọn đầy đủ thông tin bên trái!");
                    }
                ...
                if($.isNumeric(hsID) && 0 != caID && 0 != hsID && $.isNumeric(caID) && $.isNumeric(cdID)) {
                    $.ajax({
                        async: true,
                        ...
                        success: function (a) {
                            ...
                            if(a=="none") {
                                ...
                                if(obj.birth.indexOf("<?php echo $birth; ?>")!=-1) {
                                    $("#hs-info").html(obj.fullname + " ( " + obj.van + " ) " + " - " + obj.birth + " (" + obj.de + ") " + "<i class='fa fa-birthday-cake' style='margin-left:10px;font-size:1.375em;'></i>");
                                } else {
                                    $("#hs-info").html(obj.fullname + " ( " + obj.van + " ) " + " - " + obj.birth + " (" + obj.de + ") ");
                                }
                    ...
                    alert("Thiếu dữ liệu hoặc dữ liệu ko chính xác!");
                    $("#BODY").css("opacity", "1");
                    $("#popup-loading").fadeOut("fast");
                }
				...
				new_date = $(this).find("option:selected").attr("data-ngay"), $.ajax({
					...
					dem = dem-4;
					$("#so-luong").html("Có "+dem+" học sinh");
				}), $("table#list-info").delegate("tr td input#hs-made","click",function() {
                    $("table#list-info tr td input#hs-made").typeWatch({
                        captureLength: 4,
                        callback: function (value) {
                            get_form_de_thi($("#search-hs").attr("data-hsID"),$("#select-buoi").find("option:selected").val(),value);
                        }
                    });
                });
		});
...
<!--                                </tr>-->
                            ...
                            <table class="table table-2" id="list-info">
