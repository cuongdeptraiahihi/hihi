<?php
	...
	if(isset($_POST["dulieu"])) {
	    ...
	    if($dulieu=="lichhoc") {
	        ...
	        $resultx=get_all_mon_hocsinh($hsID);
	        foreach ($resultx as $key => $value) {
                echo"<li style='background:#FFF;'";if($lmID!=$value["lmID"]){echo" class='li-con'";} echo"><a href='javascript:void(0)' class='access_mon' data-lmID='$value[lmID]' title='Môn $value[name]'>$value[name]</a></li>";
            }
//            $resultx=get_all_lop_mon();
//            while($datax=mysqli_fetch_assoc($resultx)) {
//                echo"<li style='background:#FFF;'";if($lmID!=$datax["ID_LM"]){echo" class='li-con'";} echo"><a href='javascript:void(0)' class='access_mon' data-lmID='$datax[ID_LM]' title='Môn $datax[name]'>$datax[name]</a></li>";
//            }
