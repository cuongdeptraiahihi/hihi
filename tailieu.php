...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>       
        ...
        <title>TÀI LIỆU MÔN <?php echo mb_strtoupper($mon_name); ?></title>
        ...
        <script>
			$(document).ready(function() {
				...
				<?php if($cdID!=0) { ?>
				$(".main-tabs > li#tab-<?php echo $cdID; ?>").addClass("active"), $(".main-in > li#dm-<?php echo $cdID; ?>").addClass("active");
				<?php } else { ?>
				$(".main-tabs > li:first-child").addClass("active"), $(".main-in > li:first-child").addClass("active");
				<?php } ?>
				$(".main-tabs > li > a").click(function() {
					$("html,body").animate({scrollTop:0},250);
					dmID = $(this).attr("data-dmID"), tab = $(".main-in > li#dm-" + dmID), "active" != tab.attr("class") && "active" != $(this).closest("li").attr("class") && ($(".main-tabs > li.active").removeClass("active"), $(".main-in > li.active").css("display","").removeClass("active"), tab.slideDown(500).addClass("active"), $(this).closest("li").addClass("active"))
				})
				$("#MAIN .main-div .main-1>ul.main-in li ol").hover(function() {
					$("#MAIN .main-div .main-1>ul.main-in li ol").addClass("inactive");
					$(this).removeClass("inactive");
					$("#MAIN .main-div .main-1>ul.main-in li ol.inactive").stop().animate({opacity:"0.15"},250);
				},function() {
					$("#MAIN .main-div .main-1>ul.main-in li ol.inactive").stop().animate({opacity:"1"},250);
				});
			});
		</script>
            ...
            <div id="MAIN">
              	...
              	<div class="main-div back animated bounceInUp" id="main-top">
                	<div class="ask">
                        ...
                        <div class="sub-ask">
                            <ul>
                                <li><span><span style="font-size:8px">&#9899;</span> Bạn có thể tìm lại các video bài giảng của thầy để học lại</span></li>
                                <li style="margin-bottom:0"><span><span style="font-size:8px">&#9899;</span> Bao gồm có cả video có phí và miễn phí</span></li>
                            	<div class="clear"></div>
                            </ul>
                        </div>
                   	</div>
                	<div id="main-person">
                		<h1 style="line-height:98px;">Tài liệu thao khảo</h1>
                        <div class="clear"></div>
                   	</div>
                    <div id="main-avata">
                ...
                <div class="main-div animated bounceInUp">
                	<div class="main-1 back">
                        ...
                        <?php
                            $dem=0;
							$result=get_all_chuyende_con($lmID);
							while($data=mysqli_fetch_assoc($result)) {
								echo"<li id='tab-$dem'><a href='javascript:void(0)' data-dmID='$dem'>$data[title]</a></li>";
							    $dem++;
							}
                            $result=get_danhmuc($lmID,$monID);
                            while($data=mysqli_fetch_assoc($result)) {
                                echo"<li id='tab2-$data[ID_DM]'><a href='javascript:void(0)' data-dmID='$dem'>$data[title]</a></li>";
                                $dem++;
                            }
						?>
                        </ul>
                        ...
                        <?php
							...
							while($data=mysqli_fetch_assoc($result)) {
						?>
                        	<li id="dm-<?php echo $dem2; ?>">
                                <?php
									$dem=0;
									$result2=get_tailieu_dm($data["ID_CD"],0);
									while($data2=mysqli_fetch_assoc($result2)) {
										if($data2["type"]=="link") {
											echo"<ol><a href='$data2[short_con]' target='_blank'>";
										} else {
											echo"<ol><a href='https://localhost/www/TDUONG/tai-lieu/$data2[titlestring]-$data2[ID_TL].html'>";
										}
										if($data2["pic"]!="none") {
											echo"<nav class='tl-icon'><img src='https://localhost/www/TDUONG/tailieu/$data2[pic]' style='width:100%;height:100%;margin-top:-10px;' /></nav>";
										} else {
											...
											echo"<nav class='tl-con'>
												...
												if($data2["price"]!=0) { 
												echo"<p class='tl-title' style='color:#000;display:block;background:#FFF;width:100px;text-align:center;padding:5px;'>".format_price($data2["price"])."</p>";
												}
												echo"<p class='tl-intro' style='font-size:10px;'>".format_datetime($data2["dateup"])."</p>
											</nav>
											<div class='clear'></div>
										</a></ol>";
										$dem++;
