				...
				if(thang!="" && (thongbao==0 || thongbao==1) && (nhayde==0 || nhayde==1)) {
					if(confirm("Bạn có chắc chắn muốn tính điểm TB và xét nhảy đề của tháng " + thang + " ?")) {
						...
						$.ajax({
							...
							type: "post",
							url: "http://localhost/www/TDUONG/thaygiao/xuly-diem/",
							success: function(result) {
								if(result=="ok") {
									$(".status table.table").append("<tr><td colspan='2'><a href='http://localhost/www/TDUONG/thaygiao/moi-chuyen-de/" + <?php echo $lmID; ?> + "/G/'>Click vào đây để xem danh sách học sinh lên đề G</a></td></tr>");
